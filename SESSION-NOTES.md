...
## 2026-09-26 快照 · 增量 8（第 13 个玩法 EVA 突击 · 横版闯关）

**目标**：用户要「数独小游戏 + 马里奥式 EVA 闯关」→ 数独已存在（用户决定忽略），实施 platform.html
**EVA 突击**（双主角可选 · 6 关，ASCII 手工地图，Canvas 横版物理）。

**当前状态**
- 玩法 **13 个**；测试 **9 套 159 项**（platform 27 新增）；版本仍 **v1.5.1（未发版）**
- 大厅 13 张卡片（platform data-mode，配色 #b39dff，标签「动作闯关」）；构建校验清单已含 platform.html
- bot 确定性通关验证：6 关全通（1.7 分钟），真实模式 L1 无伤通关，受伤/无敌/踩怪/坠坑/超时/结算全部实测通过

**平台跳跃关键实现（与既往玩法不同的新范式）**
- 瓦片碰撞 `resolveMove`：分轴（X 先 Y 后）+ 子步防穿透；单向平台 = 下落且前帧底边不高于平台顶才吸附
- 跳跃手感三件套：可变跳高（上升按住=弱重力 1600 / 否则 2300）、土狼时间 0.08s、跳跃缓冲 0.1s
- 踩头判定：`vy>0 && 前帧底边 <= 使徒顶边+10`；碰撞盒与圆形碰撞不同（AABB vs 瓦片网格）
- 关卡 = ASCII 字符画（`#`实心 `-`单向 `^`尖刺 `o`电池 `E/F`使徒 `P`出生 `G`终点），
  测试用 `/* LEVELS-BEGIN */` 标记直接从页面提取，消除地图双份维护
- 关卡设计红线（本轮踩坑总结）：**尖刺带与沟壑落点必须隔 ≥2 格**、**孤岛必须干净**、
  **悬空砖不得压在跳跃弧线上**（起跳点/落点/跳跃顶点三处检查）——违反即静态不可达或死循环

**待办**
1. **发版待用户指定版本号**（含 platform + 无其他变更；打包后本机/线上才更新）
2. GitHub 旧 Release 清理 —— 用户已否决，勿再执行
3. 候选玩法：2048 / 记忆翻牌 / 数织 Nonogram / 推箱子 / 华容道 / 使徒追击跑酷 / 黑白棋 / 21 点 / 找茬 / 节奏打击 / NERV 基地经营

**验证环境备忘（沿用增量 6）**
- IAB 标签页 document.hidden → rAF 节流：长流程用页面内 `update(dt)` 确定性推进
- 探针须先 `pointer.on = false`（本页无此问题，弹球页有）；改地图/常量后必须**开新标签页**（旧页读旧常量）

### 增量 8 追加（2026-09-26）：道路消失 bug
- 用户实机反馈"道路不显示"→ 根因：砖块离屏缓存的**源矩形开窗**与外层 `translate(-cam)` 叠加成 2 倍位移，
  cam ≥ 960 后砖块层完全滑出屏幕；物理不受影响（站在隐形地面上）
- **验证盲区教训：所有截图都在 cam≈0 拍摄，位移为零恰好掩盖 bug——视觉验证必须覆盖非零摄像机位置**
- 修法：缓存整层按世界坐标绘制，开窗交给 translate；已在第 4 关 cam=1393 处截屏验证

---

## 2026-09-26 快照 · 增量 9（v1.5.2 发版：EVA 突击上线）

- **v1.5.2 已发版**（本地 + 线上）：四处一致（serve.py = 包内 = 本机安装 = Release tag）
  Release: https://github.com/Honor-Boop/asuka-rei-lianliankan/releases/tag/v1.5.2
  内容：第 13 个玩法 EVA 突击 + 手感画面优化（镜头平滑/缓存/固定步长/人设精细化）+ 修复（道路消失/黑脸/暴走已在 v1.5.1 修）
- **⚠ 用户版本规则（已入长期记忆 eva-release-versioning）**：发版版本号只递增 patch 位——
  v1.5.2 → 1.5.3 → … → v1.5.9 → 之后才 v1.6.0。**不要因新增玩法自行跳 minor**（曾定 v1.6.0 被纠正）
- 本轮另修复：平台页黑脸（下颌影未重置 fillStyle）+ 头发路径分解；3 倍放大截图验收双主角

...
## 2026-09-20 快照 · 增量 6（第 12 个玩法 AT 力场弹球 + 暴走失效缺陷）

**目标**：用户问「再推荐一些还没有可以做出来的小游戏」并连发两次「继续」→ 给出候选清单，直接开工做出
**物理技巧类**（合集此前唯一空白类型）新玩法 **AT 力场弹球**（`breakout.html`）。

**已确认决定**
- 新玩法一律 Canvas 实时渲染、零新素材（本次纯程序化绘制：装甲块/AT 力场档板/阳电子炮）
- 与弹幕射击/塔防共用同一套页面骨架与 `__decoSyncRate` 联动；同步率仍为主线机制

**当前状态**
- 玩法 **12 个**；测试 **8 套 132 项**（breakout 36 新增）
- 大厅 12 张卡片（AT 弹球 data-mode=breakout，配色 #b0ff5c，标签「物理技巧」）
- 平衡（探针实测）：見習い 5/5 关、正規隊員 6/6 关均可通关，单局约 16 分钟，暴走触发 9-16 次

**本轮修掉的 5 处缺陷（重要）**
1. **暴走永远无法触发**（`updateSync` 先衰减后判定 + 增益被 clamp 在恰好 100）——
   **弹球与已发布的 v1.5.0 弹幕射击同源**，即线上版「暴走」功能一直没生效；已修 + 两游戏加守护测试
2. 击碎砖块时提前 return 导致**跳过反弹**（有无穿透都能贯穿整列）
3. 过关瞬间重建数组 → 旧循环下标读新数组 **TypeError 崩溃**
4. 正中接球反射角为 0 → **垂直死循环**（整列清空后球在空列无限弹跳，关卡打不完）
5. 残局周期性轨道打不到最后装甲 → 加轨道微调防呆
- 教训：纯函数测试全绿 ≠ 集成正确。**"先判定后衰减"这类帧序依赖必须写模拟帧序的守护测试**

**待办**
1. **弹幕射击的暴走修复尚未到线上**：需要发版（v1.6.0 含弹球 + 暴走修复）才能进安装包与 Release —— 等用户授权
2. GitHub 旧 Release 清理 —— 用户已明确否决，勿再执行
3. 候选玩法（尚未实现，用户可随时挑）：2048 / 记忆翻牌 / 数织 Nonogram / 推箱子 / 华容道 / 使徒追击跑酷 /
